...
type StartContainerFromCheckpointRequest struct {
	...
	Checkpoint string  `protobuf:"bytes,2,opt,name=checkpoint,json=checkpoint,proto3" json:"checkpoint,omitempty"`
	// 
...
func _RuntimeService_StartContainerFromCheckpoint_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	if interceptor == nil {
		return srv.(RuntimeServiceServer).StartContainerFromCheckpoint(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/runtime.v1alpha2.RuntimeService/StartContainerFromCheckpoint",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RuntimeServiceServer).StartContainerFromCheckpoint(ctx, req.(*StartContainerFromCheckpointRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
func (m *StartContainerFromCheckpointRequest) MarshalTo(dAtA []byte) (int, error) {
	...
	if len(m.Checkpoint) > 0 {
		dAtA[i] = 0x12
		i++
		i = encodeVarintApi(dAtA, i, uint64(len(m.Checkpoint)))
		i += copy(dAtA[i:], m.Checkpoint)
	}
	if len(m.CheckpointDir) > 0 {
		dAtA[i] = 0x1a
		i++
		i = encodeVarintApi(dAtA, i, uint64(len(m.CheckpointDir)))
		i += copy(dAtA[i:], m.CheckpointDir)
	}
	return i, nil
}
...
func (this *StartContainerFromCheckpointRequest) String() string {
	...
	s := strings.Join([]string{`&StartContainerFromCheckpointRequest{`,
		`ContainerId:` + fmt.Sprintf("%v", this.ContainerId) + `,`,
...
func (m *StartContainerFromCheckpointRequest) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: StartContainerFromCheckpointRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: StartContainerFromCheckpointRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
